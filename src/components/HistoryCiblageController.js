...
import {
  ...
  fetchControlers,
  fetchEnquetersForController,
  fetchMenagesForController,
  fetchZones,
  fetchLocalites,
  fetchCommunes,
...
const HistoryCiblageSupervisor = ({ user, setLoading, t }) => {
  ...
  useEffect(() => {
    setTimeout(() => {
      (async () => {
        ...
        let lclts = [];
        let cmns = [];
        mngs = await fetchMenagesForController(user);
        conces = await fetchConcessionsForSupervisor(user);
        enqs = await fetchEnquetersForController(user);
