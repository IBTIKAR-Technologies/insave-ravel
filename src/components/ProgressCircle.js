...
export default class ProgressCircle extends Component {
  ...
  render() {
    const {
      thickness, unfilledColor, children, style,
    } = this.props;
  ...
  animateChange = (value) => Animated[this.animationMethod](this.state.animatedValue, {
    toValue: value,
  ...
  renderHalfCircle = ({ isFlipped = false } = {}) => {
    const {
      size, color, thickness, value, style,
    } = this.props;
    const valueToInterpolate = value.constructor.name === 'AnimatedValue'
      ? value
