...
const Listes = function ({ componentId }) {
  ...
  const initialize = useCallback(async () => {
    const all = global.realm.objects('listes').filtered('executed == true');
    /*       const clientcentre = global.realm.objects('clientscentres')[0];
            console.log('clientcentre', clientcentre);
            for (let i = 0; i < clientcentre.listesIds.length; i++) {
              const listeId = clientcentre.listesIds[i];
              console.log('listeId.toString()', listeId.toString())
            } */
  ...
  const handleViewListe = liste => {
    navigate(componentId, screenNames.ViewListe, t('listes_view_title'), { setSuccess, liste });
  };
  ...
  return (
    <LinearGradient
      ...
      <View style={styles.info}>
        <Text style={styles.header}>{t('listesa')}</Text>
      ...
      <View style={[styles.info, styles.flatBlock]}>
        <FlatList
          ...
          renderItem={({ item, index }) => {
            const { _id, membersCount, qte_ble, qte_rakel, qte_ble_vendu, qte_rakel_vendu, ordre } =
              item;

            return (
              <List.Item
                key={_id}
                title={`${t('liste')} ${ordre} | ${membersCount} ${t('members')}`}
                description={`${t('ble_vs')}: ${qte_ble_vendu}/${qte_ble} \n${t(
                  'rakel_vs',
                )}: ${qte_rakel_vendu}/${qte_rakel}`}
                right={() => (
                  <View style={styles.right}>
                    <Button
                      onPress={() => handleViewListe(item)}
                      style={styles.button}
